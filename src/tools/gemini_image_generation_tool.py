...
# --- Constants ---
OUTPUT_IMAGE_CACHE_DIR = "./output_cache"
...
def generate_gemini_image(
    ...
    - 多模態輸入：`image_inputs` 中可以處理檔案路徑字串（以逗號分隔多個路徑）。
    ...
    **注意：** 模型有時可能只輸出文字或停止生成。
    ...
    if image_inputs:
        if isinstance(image_inputs, str):
            # 如果是字串，分割成路徑列表
            image_paths = [path.strip() for path in image_inputs.split(',') if path.strip()]
            processed_image_inputs = image_paths
        else:
            processed_image_inputs = image_inputs if isinstance(image_inputs, list) else [image_inputs]

    print(f"Gemini Image Gen Tool (Client Mode): Received prompt='{prompt[:50]}...', {len(processed_image_inputs)} image inputs.")
    
    # 準備 contents 列表 - 首先加入文本提示
    if processed_image_inputs:
        # 圖生圖／圖片編輯
    ...
    # 處理圖片輸入
    if processed_image_inputs:
        for img_input in processed_image_inputs:
            try:
                # 處理圖片輸入並獲取字節和MIME類型
